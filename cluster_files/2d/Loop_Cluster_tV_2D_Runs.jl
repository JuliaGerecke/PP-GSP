...
"""
Run loop scaling benchmarks for 2D tV model on square lattice with OBC.
...
"""
function loop_tv_2d_benchmarks(tile_width, scaled_lattice_width; use_initial_circuit=false)
    ...
    hamiltonian = SystemHamiltonian(:tV)
    ...
    results = run_loop_scaling_benchmarks_2d(
        ...
        mw_sequences=[[5]],
        mac_sequences=[[1e-4]],
        min_iters_for_stagnation_sequences=[[25]],
        ...
        output_dir="PP-GSP/cluster_data/Benchmark_2D_$(hamiltonian)_$(backend)_grad_$calc_grad_kwargs",
        verbose=false,
        vscore=false,
        overlap_func=overlapwithneel,
        hamiltonian_kwargs=(t=1.0, V=1.0),
...
for i in scaled_width
    loop_tv_2d_benchmarks(2, i; use_initial_circuit=false)  # 2x2 → ixi (4 → i^2 qubits)
end
