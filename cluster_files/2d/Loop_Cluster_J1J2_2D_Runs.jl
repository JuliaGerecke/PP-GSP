...
"""
Run loop scaling benchmarks for 2D J1-J2 model on square lattice with OBC.

The J1-J2 model has:
- J1: Heisenberg interaction on nearest neighbors (horizontal and vertical)
- J2: Heisenberg interaction on diagonal neighbors
...
"""
function loop_j1j2_2d_benchmarks(tile_width, scaled_lattice_width; use_initial_circuit=false)
    ...
    hamiltonian = SystemHamiltonian(:J1J2_2d_obc)
    ...
    results = run_loop_scaling_benchmarks_2d(
        ...
        mw_sequences=[[4],[5]],
        mac_sequences=[[1e-4],[1e-5]],
        min_iters_for_stagnation_sequences=[[40],[40]],
...
for i in scaled_width
    loop_j1j2_2d_benchmarks(2, i; use_initial_circuit=false)  # 2x2 → ixi (4 → i^2 qubits)
end
